...
- [`docs/index.html`](docs/index.html): 統合スクリーナー。
  ...
  - 銘柄モーダル: 銘柄詳細（**決算日**・**需給/水準: サポート20日/レジスタンス20日/ATR14(円・%)/52週高値位置/200日線乖離/レンジ位置**・**過熱警戒バッジ**）＋**価格帯別ルールの利確/損切（ATR損切。利確=赤・損切=青）**＋**推奨株数**＋決算警告。AI戦略は**参考情報（売買指示には未採用）**と明示。「最新AI実行」ボタンは**常時有効**（プロンプトをコピーしてGeminiへ）。
