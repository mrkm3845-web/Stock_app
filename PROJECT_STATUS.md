...
- [`docs/index.html`](docs/index.html): 統合スクリーナー。
  - 一覧の列: **No / 銘柄 / スコア / エントリー(＋過熱度) / 増加率 / 決算 / 現在値 / 割安度 / PER / 利回り / PBR / ROE**。エントリーは `ctx`＋`entry_guard` からフロントで全銘柄ぶん算出（`common/features.compute_entry_plan` と同式）。**列見出しクリックでソート（昇順⇄降順トグル、▲▼表示）**、上部プルダウンと同期。スマホでは二次列（現在値/割安度/PER/利回り/PBR/ROE）を自動非表示（`d-none d-md/lg-table-cell`）。銘柄列は横スクロール時に固定（sticky）。
  ...
  - デフォルト並び替え: **総合スコア順**（スコア→出来高増加率→5日平均代金）。「エントリー判定順」「決算が近い順」「AI推奨順」も選択可。
  - 銘柄モーダル: 銘柄詳細（**決算日**・**需給/水準: サポート20日/レジスタンス20日/ATR14(円・%)/52週高値位置/200日線乖離/レンジ位置**・**過熱警戒バッジ**）＋**価格帯別ルールの利確/損切（ATR損切。利確=赤・損切=青）**＋**推奨株数**＋決算警告。AI戦略は**参考情報（売買指示には未採用）**と明示。AI判定が注意/回避/様子見のときは**「注意の理由」コールアウト**を表示し、バッジにも理由ツールチップを付与。「最新AI実行」ボタンは**常時有効**（プロンプトをコピーしてGeminiへ）。
