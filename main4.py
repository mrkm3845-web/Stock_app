...
def scan_market(stocks_list, market_name, max_workers=6):
    results = []
    print(f">> 【{market_name}】全 {len(stocks_list)} 銘柄のスキャン開始...")
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(analyze_single_stock, s, market_name): s for s in stocks_list}
...
def save_to_sqlite(all_stocks):
    ...
    for s in all_stocks:
        ...
        """, (today_str, s["code"], s["name"], s["market"], s["sector"], s["price"],
              s["graham_price"], s["discount_rate"], s["mix_index"], s["per"], s["pbr"],
              s["roe"], s["op_margin"], s["div_yield"]))
    conn.commit()
    conn.close()


# 5. 日別JSON保存（最新データ ＆ 過去履歴）
