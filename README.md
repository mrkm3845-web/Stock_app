...
日本株（プライム／スタンダード 約3,100銘柄）を毎日スキャンし、**技術スコアで候補を絞り、AI（Gemini）が順位付け**します。AIが `recommend` と判定した銘柄のみを**推奨**として提示し、無理に件数は埋めません（該当が無い日は「本日は推奨なし」）。バックテストによる検証と、`docs/strategy_params.json` への**ガード付き自動反映**までを単一リポジトリで行います。
